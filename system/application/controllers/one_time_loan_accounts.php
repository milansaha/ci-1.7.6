<?php
/** 
	* One time loan account Controller Class.
	* @pupose		Manage one time loan information
	*		
	* @filesource	\app\model\one_time_loan_accounts.php
	* @package	microfin 
	* @subpackage	microfin.model.one_time_loan_accounts
	* @version      $Revision: 1 $
	* @author       $Author: Amlan Chowdhury $	
	* @lastmodified $Date: 2011-04-11 $
*/
class One_time_loan_accounts extends MY_Controller {
	...
	function authorization_index(){
		$cond= "";
		$session_data = $this->session->userdata('one_time_loan_accounts.index');
	
		if(isset($_POST['cbo_samity'])){
			$cond['samity'] = $_POST['cbo_samity'];	
			$sessionArray = array( 'one_time_loan_accounts.authorization_index'=>array('cbo_samity'=>$cond['samity']));
			$this->session->unset_userdata('one_time_loan_accounts.authorization_index');
		...
		if(isset($_POST['loandata']))
		{
			...
			{				
				...
				{				
					$data['one_time_loan_accounts'][$i]['id']=$row['id'];
					$data['one_time_loan_accounts'][$i]['is_authorized']=0;
					if(!empty($row['is_authorized']))
					{
						$data['one_time_loan_accounts'][$i]['is_authorized']=$row['is_authorized'];
					}							
					$data['one_time_loan_accounts'][$i]['authorized_by']=$user['id'];
					$data['one_time_loan_accounts'][$i]['authorization_date']=date('Y-m-d');
					$i++;					
				}				
			}
			else
			{				
				...
				{				
					$data['one_time_loan_accounts'][$i]['id']=$row['id'];
					$data['one_time_loan_accounts'][$i]['is_authorized']=1;
					$data['one_time_loan_accounts'][$i]['authorized_by']=$user['id'];
					$data['one_time_loan_accounts'][$i]['authorization_date']=date('Y-m-d');
					$i++;					
				}
			}
			//echo "<pre>";print_r($data['one_time_loan_accounts']);die;
			if($this->Loan->authorized($data['one_time_loan_accounts']))
			{
				$this->session->set_flashdata('message','Loan information has been authorized successfully');
				redirect('/one_time_loan_accounts/authorization_index/');
			}
		}
		...
		$config['base_url'] = site_url('/one_time_loan_accounts/authorization_index/');
		$config['total_rows'] = $total;
		$config['per_page'] = ROW_PER_PAGE;
		$this->pagination->initialize($config);			
		$data['samities'] = $this->Samity->get_samity_list($this->get_branch_id());
		//Loading data by model function call
		$data['one_time_loan_accounts']=$this->Loan->get_unauthorized_loan_list(ROW_PER_PAGE, (int)$this->uri->segment(3),$cond);
		$data['counter'] = (int)$this->uri->segment(3);
		// Loading samity list which will be used in combo box	
				
		$this->layout->view('/one_time_loan_accounts/authorization_index',$data);
	}
	...
	function index(){
		...
		if(isset($_POST['txt_name']) || isset($_POST['cbo_samity']) || isset($_POST['cbo_loan_status'])){
			...
			$sessionArray = array( 'one_time_loan_accounts.index'=>array('name'=>$cond['name'],'cbo_samity'=>$cond['samity'],'cbo_loan_status'=>$cond['loan_status']));
			$this->session->unset_userdata('one_time_loan_accounts.index');
			$this->session->set_userdata($sessionArray);
		} elseif(is_array($session_data)) {			
		...
		// load pagination class
		$this->load->library('pagination');
		$total = $this->Loan->row_count_one_time_loans($cond);

		//Initializing Pagination
		$config['base_url'] = site_url('/one_time_loan_accounts/index/');
		$config['total_rows'] = $total;
		$config['per_page'] = ROW_PER_PAGE;
		...
		//Loading data by model function call
		$data['one_time_loan_accounts']=$this->Loan->get_list_one_time_loans(ROW_PER_PAGE, (int)$this->uri->segment(3),$cond);
		$data['counter'] = (int)$this->uri->segment(3);
                $data['title']='One Time Loan Accounts';
		$data['headline']='One Time Loan Accounts';
		$this->layout->view('/one_time_loan_accounts/index',$data);
	}
	...
	function add(){
		...
		$is_validate_error = false;		
			...
			$cbo_current_status = $this->input->post('cbo_current_status');  
                        $cbo_loan_period_in_month = $this->input->post('cbo_loan_period_in_month');
			...
			if ($this->form_validation->run() == TRUE){				
				...
				if($this->Loan->add($data)){
                                    ...
                                    redirect('/one_time_loan_accounts/index/');
				}
			} 
			else {			
                                 //print('hello2');die;
		...
		if($is_validate_error and isset($member_id) and !empty($member_id)){
			...
			$data['row']->current_status = isset($cbo_current_status)?$cbo_current_status:"";
                        $data['row']->cbo_loan_period_in_month = isset($cbo_loan_period_in_month)?$cbo_loan_period_in_month:"";
		...
		$data['title']='Add One Time Loan Account';
                $data['headline'] = 'Add';
		$this->layout->view('/one_time_loan_accounts/save',$data);
	}	
	...
	{			
		...
		if($is_validate_error and isset($member_id) and !empty($member_id)){
            ...
            $data['title']='Edit One Time Loan Account';
            $data['headline'] = 'Edit';
            //If data is not posted or validation fails, the add view is displayed
            $this->layout->view('one_time_loan_accounts/save',$data);
	}
	...
	{						
		...
		$data['number_of_installment']=1;
		$data['loan_period_in_month']=$this->input->post('cbo_loan_period_in_month');
		...
		$data['mode_of_interest']='DAILY_PER_THOUSAND';
		$data['interest_calculation_method']='FLAT';
	...
	function _prepare_validation(){
		...
		$this->form_validation->set_rules('cbo_loan_period_in_month','Loan period in month','trim|xss_clean|numeric|required');
	...
	function _load_combo_data($load_combo_conditon = array()){
		...
		$data['loan_period_in_month'] = array(''=>'--Select--','1'=>'1','3'=>'3','4'=>'4','6'=>'6','12'=>'12');
                $data['current_status'] = array('0'=>'Active','1'=>'Inactive','2'=>'Closed');               
		return $data;
	}	
