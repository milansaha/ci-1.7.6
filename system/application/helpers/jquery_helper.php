...
/*
 * created by saroj on 23/03/2011
*/


if ( ! function_exists('ajax_form_for_report')) {
	function ajax_form_for_report($action = '',$return_div, $loader_div = '', $attributes = array(),$hidden=array()) {
		$obj =& get_instance();
		$obj->load->helper('form');
		if ( ! array_key_exists('onsubmit', $attributes)) {
			$attributes['onsubmit'] = "";
		...
		if(!empty($loader_div)) $attributes['onsubmit'] .='$(\''.$loader_div.'\').fadeIn();';
		$attributes['onsubmit'] .='$.ajax({';
		$attributes['onsubmit'] .='type: \'post\', ';
		$attributes['onsubmit'] .='data: $(this).serialize(),';
		$attributes['onsubmit'] .='url: this.action, ';
		$attributes['onsubmit'] .='success: function(data) {$(\''.$return_div.'\').html(data).fadeIn();show_print_toolbar();}, ';
		$attributes['onsubmit'] .='error: function(data) {} ';
		if(!empty($loader_div)) $attributes['onsubmit'] .=', complete: function(){ $(\''.$loader_div.'\').fadeOut();}';
		$attributes['onsubmit'] .='});';		
...
/*
 * created by Anis Alamgir on 10/04/2011
*/


if ( ! function_exists('ajax_form')) {
	function ajax_form($action = '',$return_div, $loader_div = '', $attributes = array(),$hidden=array()) {
		$obj =& get_instance();
		$obj->load->helper('form');
		if ( ! array_key_exists('onsubmit', $attributes)) {
			$attributes['onsubmit'] = "";
		...
		$attributes['onsubmit'] .='data: $(this).serialize(),';
		$attributes['onsubmit'] .='url: this.action, ';
		$attributes['onsubmit'] .='success: function(data) {$(\''.$return_div.'\').html(data).fadeIn();}, ';
		$attributes['onsubmit'] .='error: function(data) {} ';
		if(!empty($loader_div)) $attributes['onsubmit'] .=', complete: function(){ $(\''.$loader_div.'\').fadeOut();}';
		$attributes['onsubmit'] .='});';		
		$attributes['onsubmit'] .='return false;';
...
/*
 * created by saroj on 20/03/2011
ajax_anchor creates an anchor tag that will jquery ajax load the contents into the specified selector's element(s). 
$return_div is the where the content returned from the GET is put and can be any appropriate jQuery selector.
*/
if ( ! function_exists('ajax_anchor')) {
	function ajax_anchor($return_div, $loader_div = '', $uri = '', $title = '', $attributes = array()) {
		$obj =& get_instance();
		$obj->load->helper('url');
		if ( ! array_key_exists('onclick', $attributes)) {
			$attributes['onclick'] = "";
		...
		//$attributes['onclick'] .='$.ajax({url: this.href,success: function(data) {$(\''.$returnSelector.'\').html(data);alert(\'Load was performed.\');}});return false;';
		if(!empty($loader_div)) $attributes['onclick'] .='$(\''.$loader_div.'\').fadeIn();';
		$attributes['onclick'] .='$.ajax({';
		$attributes['onclick'] .='url: this.href, cashe: false, ';
		$attributes['onclick'] .='success: function(data) {$(\''.$return_div.'\').html(data);}, ';
		$attributes['onclick'] .='error: function(data) {alert(\'Error performing operation. Please check your network connectivity.\');}';
		if(!empty($loader_div)) $attributes['onclick'] .=', complete: function(){ $(\''.$loader_div.'\').fadeOut();}';
...
if ( ! function_exists('ajax_cascade_select')) {
	function ajax_cascade_select($own_id,$options,$target_id,$url, $Options = array()) {
		$acJs = <<<JS
				<script type="text/javascript">
					$(document).ready(function() {
						$('#$own_id').change(function() {
							 var division_id = this.value;				
								$.post("$url", {division_id: division_id }, function(data) { 								
										$('#$target_id').html("<option value=''>--Select--</option>");
...
JS;
		...
		$acInput = form_dropdown($own_id,$options,null,"id=$own_id");
		return ($acJs . $acInput);
	}
}

//Implemented by -Saroj
//autocomplete textbox with result callback
if ( ! function_exists('form_input_autocomplete_text')) {
	function form_input_autocomplete_text($data = '', $uriOrData = '', $resultCallback=null) {
		$obj =& get_instance();
		$obj->load->helper(array('form', 'url'));
		if(empty($acOptions))
		{
			//Loading default format options when empty
			$acOptions=array('minChars'=>2,'matchContains'=>'word','highlightItem'=> FALSE,'dataType'=> 'json');
		}
		$formattedAcOptions = _format_jquery_options($acOptions);
		if (is_string($data)) {
			$name = $data;
		...
		//Implementing the result callback method -- Saroj
		$resultJS="";
		if($resultCallback)
		{
			$resultJS="$resultCallback(item);";
		}
		if ( is_string($uriOrData) ) {
			$url  = $uriOrData . "/";
			$acJs = <<<JS
				<script type="text/javascript">
					$(document).ready(function() {
						function format(data) {
							...
							return "<strong>"+data.name+"</strong>" + "<br><span style='font-size: 80%;'>" + data.id + "</span>";
						}
						$('#${data['id']}').autocomplete('${url}',{ 
							minChars: 0,
							dataType: "json",
							autoFill: true,
							matchContains: "word",
...
JS;
			...
			$acJs = <<<JS
				<script type="text/javascript">
					$(document).ready(function() {
						var staticData = "${arrData}".split("|");
						$('#${data['id']}').autocomplete(staticData,{ ${formattedAcOptions} }).result(function(e, item) {
							$resultJS
						});
					});
				</script>
