<?php
class Saving_advance_register extends MY_Model {
    ...
    {
	$query = $this->db->query("
                        SELECT s.id,p.name as product_name,sa.name as samity_name,m.name as member_name,br.name as branch_name,s.date,s.amount,s.amount_type
			FROM products as p
                        LEFT JOIN saving_advance_register as s                        
        ...
        ");  
//        $query = $this->db->get('saving_advance_register', $offset, $limit);
        return $query->result();
    }
	
	function row_count()
	{
		return $this->db->count_all_results('saving_advance_register');
	}
	
    function add($data)
    {
        return $this->db->insert('saving_advance_register', $data);
    }

    function edit($data)
    {
        return $this->db->update('saving_advance_register', $data, array('id'=> $data['id']));
    }
	
	function read($saving_advance_register_id)
    {
        $query=$this->db->getwhere('saving_advance_register', array('id' => $saving_advance_register_id));
		return $query->result();
    }
	
	function delete($saving_advance_register_id)
	{
		return $this->db->delete('saving_advance_register', array('id'=> $saving_advance_register_id));
	}
