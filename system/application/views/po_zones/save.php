<?php
	...
	if($action=='edit')
	{
		$hidden_input=array('txt_zone_id'=>$row->id);
		$class_name = 'class="formTitleBar_edit"';
	}else{$class_name = 'class="formTitleBar_add"';}
	echo form_open("po_zones/$action",'',$hidden_input);
?>
<fieldset>
		...
		<tr>
			...
			<td class="formTitleBar">
				<div style="float:right;">
					...
					<?php echo form_button(array('name'=>'button','id'=>'button','value'=>'true','type'=>'reset','content'=>'Cancel','class'=>'cancel_buttons','onclick'=>"window.location.href='".site_url('po_zones')."'"));?>
				</div>
			</td>
		</tr>
		<tr>
			<td width="60%">
				<div class="formContainer">
					<ol>
						...
						<li>
							<label for="cbo_area_list">Area:<span class="required_field_indicator">*</span></label>
                            <div class="form_input_container">
							<?php echo form_multiselect('cbo_area_list[]', $area_infos, set_value('cbo_area_list', (isset ($selected_area)?$selected_area:'')),'id="cbo_area_list" class="input_select" size="10"'); ?>
                            <div class="label_adder"><?php echo form_label(img(array('src'=>base_url().'media/images/add.png','border'=>'0','alt'=>'Add Area','width'=>'12px')), 'add_area', array('class'=>'addimglink','style'=>'border:none;','title'=>'Add Area','onclick'=>"window.location.href='".site_url('po_areas/add')."'"));?></div>
							<?php echo form_error('cbo_area_list[]'); ?>
                            </div>
						</li>
					</ol>
				</div>
