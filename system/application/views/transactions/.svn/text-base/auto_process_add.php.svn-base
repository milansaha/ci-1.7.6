			...
			$("input[name^=ch_" + type + "_" + cat + "_member]").each(function()
			{
				if(!this.disabled){						
					...
					$("#txt_" + type + "_amount_"+c).attr("value","0.00");
						...
						if(cat == "partial") {
				        	$("#txt_" + type + "_amount_"+c).attr("value",'');
...
<fieldset>
	...
	<table class="auto_process" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
			...
			<td  colspan="4">&nbsp;</td>
			<td><input type="checkbox" id="ch_loan_full_all" value="loan_all" <?php echo $full_check_flag?>></td>
			...
			<td colspan="4">&nbsp;</td>
			<td><input type="checkbox" id="ch_savings_full_all" value="savings_all" <?php echo $full_check_flag?>></td>
		...
		if(isset($auto_process) and !empty($auto_process)){
			//echo '<pre>';
			//print_r($auto_process);
			foreach($auto_process as $member_info)
			{
				$due = (isset($member_info->due_or_advance) && $member_info->due_or_advance>0)?$member_info->due_or_advance:'0';
				$advance = (isset($member_info->due_or_advance)&& $member_info->due_or_advance<0)?(-($member_info->due_or_advance)):'0';				
			...
			?>
				...
				<input name="loan_due[<?php echo $member_info->loan_id?>]" value="<?php echo ($due>0)?$due:'0'?>" type="hidden">
				<input name="loan_advance[<?php echo $member_info->loan_id?>]" value="<?php echo ($advance>0)?$advance:'0'?>" type="hidden">
				<input name="h_loan_amount[]" id="h_loan_amount_<?php echo $row?>" value="<?php echo ceil($member_info->installment_amount)?>" type="hidden">
				...
				<td width="20px" align="center">
					...
					?>
					<input type="checkbox" id="ch_attendence" class="ch_attendence" name="ch_attendence[<?php echo $member_info->member_id?>]" value="1">
				<?php }else{?>
					-					
				<?php }?>
				</td>	            
	            <td width="50px" ><?php echo empty($member_info->customized_loan_no)?'-':$member_info->customized_loan_no?></td>
			    <td width="30px" align='center'><?php echo ($member_info->installment_amount <= 0 ) ?'-':$member_info->installment_amount?></td>
	            <td width="30px" align='center'><?php echo ($due>0)?$due:'-'?></td>
				<td width="30px" align='center'><?php echo ($advance>0)?$advance:'-'?></td>
				<td width="20px"><input name="ch_loan_full_member[]" <?php echo empty($member_info->loan_id)?'':$full_check_flag?> id="ch_loan_full_member_<?php echo $row?>" value="<?php echo $row?>" <?php echo set_checkbox('ch_loan_full_member[]', $row); ?> <?php echo $loan_disabled?> type="checkbox" onclick="jqCheck('<?php echo $row?>','loan','full');" ></td>
				...
				<td width="30px">
					<?php if(!empty($member_info->loan_id)){?>
					<input id="txt_loan_amount_<?php echo $row?>" class="auto_process_input" name="txt_loan_amount[<?php echo $member_info->loan_id?>]" size="3" type="text" readonly="readonly" value="<?php echo ($member_info->installment_amount <= 0 ) ?'0':ceil($member_info->installment_amount)?><?php //echo isset($loan_amount_value[$row - 1])?$loan_amount_value[$row - 1]:$loan_reset_value; ?>">
					<?php }else{?>
					<input  name="txt_loan_amount[]" value='0' type='hidden'></td>
					<?php }?>
				</td>
			    <td width="8px">|</td>
				<td width="40px"><?php echo empty($member_info->savings_id)?'-':$member_info->savings_acc?></td> 
				<td width="25px" align='center'><?php echo ($member_info->weekly_savings <= 0 )  ?'-':$member_info->weekly_savings?></td> 
				<td width="20px"><input id="ch_savings_full_member_<?php echo $row?>" <?php echo empty($member_info->savings_id)?'':$full_check_flag?> name="ch_savings_full_member[]" <?php echo set_checkbox('ch_savings_full_member[]', $row); ?> value="<?php echo $row?>" <?php echo $savings_disabled?> type="checkbox" onclick="jqCheck('<?php echo $row?>','savings','full');"></td>
				...
				<td width="40px">
				<?php if(!empty($member_info->savings_id)){?>				
					<input id="txt_savings_amount_<?php echo $row?>" name="txt_savings_amount[<?php echo $member_info->savings_id?>]" readonly="readonly" size="3" type="text" class="required auto_process_input"  value="<?php echo ($member_info->weekly_savings <= 0 ) ?'0':ceil($member_info->weekly_savings)?><?php //echo isset($savings_amount_value[$row - 1])?$savings_amount_value[$row - 1]:$savings_reset_value; ?>"></td>
				<?php }else{?>
				<input name="txt_savings_amount[]" value='0' type='hidden'></td>
				<?php }?>
				<td width="10px">
					<?php 
					if($member_id_old!=$member_info->member_id){
					...
					?>
					<input id="txt_skt_amount_<?php echo $row?>" name="txt_skt_amount[]" readonly="readonly" size="2" type="text" value="<?php echo isset($member_info->skt_amount)?$member_info->skt_amount:0; ?>" class="auto_process_input">
				   <?php }else{?>
				   -
		...
		<tr>
			...
			<th><div id="total_loan_amount"><?php echo ceil($total_installment_amount);//printf("%.2f",(is_array($loan_amount_value))? array_sum($loan_amount_value):'0.00')?></div></th>
			...
			<th><div id="total_savings_amount"><?php echo $total_fixed_savings;//printf("%.2f",(($savings_amount_value))? array_sum($savings_amount_value):'0.00')?></div></th>
		...
		<?php echo form_submit(array('name'=>'submit','id'=>'submit','class'=>'submit_buttons positive'),'Preview');?>
		</td></tr>
	</table>
	</center>
</fieldset>
