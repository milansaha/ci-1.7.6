			...
			function(data)
				{
                    ...
                    $('#cbo_village').append('<option value = "">--------SELECT----------</option>');
...
$("#cbo_union").change(
	function(){
		...
		var selected_union_id = $("#cbo_union").val();
        
		$.post("<?php echo site_url('po_working_areas/ajax_for_get_village_by_union') ?>", { union_id: selected_union_id},
			function(data)
				{
					$('#status').html("");
					$('#cbo_village').empty();
					...
					else
					{
						...
						for(var i = 0; i < data.village.id.length; i++)
						{
							
                            $('#cbo_village').append('<option value = \"' + data.village.id[i] + '\">' + data.village.name[i] + '</option>');
...
<?php
    ...
    if(isset ($divisions) and !empty($divisions))
    {
    ...
    if(isset ($thanas) and !empty($thanas))
    {
    ...
    if(isset ($unions) and !empty($unions))
    {
    ...
    if(isset ($villages) and !empty($villages))
    {
        foreach($villages as $village_row)
        {
            $village_options[$village_row->id]=$village_row->name;
        }
    }
?>
	<?php
		...
		if($action=='edit')
		{
			$hidden_input=array('working_area_id'=>$row->id);
			$class_name = 'class="formTitleBar_edit"';
	}else{$class_name = 'class="formTitleBar_add"';}
		echo form_open("po_working_areas/$action",'',$hidden_input);
	?>
<fieldset>
		...
		<tr>
			...
			<td class="formTitleBar">
				<div style="float:right;">
					...
					<?php echo form_button(array('name'=>'button','id'=>'button','value'=>'true','type'=>'reset','content'=>'Cancel','class'=>'cancel_buttons','onclick'=>"window.location.href='".site_url('po_working_areas')."'"));?>
				</div>
			</td>
		</tr>
		<tr>
			<td width="60%">
				<div class="formContainer">
					<ol>  
						...
						<li>
							<label for="cbo_village">Village:<span class="required_field_indicator">*</span></label>
                            <div class="form_input_container">
							<?php echo form_dropdown('cbo_village', $village_options, set_value('cbo_village', (isset($row->village_or_block_id)?$row->village_or_block_id:"")),'id="cbo_village" class="input_select"'); ?>
                            <div class="label_adder"><?php echo form_label(img(array('src'=>base_url().'media/images/add.png','border'=>'0','alt'=>'Add Village','width'=>'12px')), 'add_village_or_block', array('class'=>'addimglink','style'=>'border:none;','title'=>'Add Village','onclick'=>"window.location.href='".site_url('po_village_or_blocks/add')."'"));?></div>
							<?php echo form_error('cbo_village'); ?>
                            </div>
						</li> 						
					</ol>
				</div>
