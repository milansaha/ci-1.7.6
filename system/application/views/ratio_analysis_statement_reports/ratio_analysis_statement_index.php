...
function validate(form) 
{
	...
	//alert(cbo_year);
	var cbo_project = form.cbo_project.value;		
	//alert(cbo_month);
	...
	if(cbo_branch == "") {		
		...
		inlineMsg('cbo_project','<strong>Error</strong><br />You must select a project.',2);
		return false;
	}
	else if(cbo_month == "") {		
...
?>
<?php echo ajax_form_for_report('additional_reports/ajax_ratio_analysis_statement_report','#report_container',null,array('onsubmit'=>'if(validate(this)==false) return false;'));?>
<div style="border-bottom:solid 0px #dedede;width:100%;float:left;">
	...
	<table border="0" class="reportLayout" width="auto" cellspacing="0px" cellpadding="0">	
		...
		<td>
			<label for="cbo_project">Projects/Fund:<em>&nbsp;</em></label>			
			<?php echo form_dropdown('cbo_project', $project_options,"",'id="cbo_project"'); ?>			
		</td> 		
		<td>
