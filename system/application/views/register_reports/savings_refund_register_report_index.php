...
<?php
	...
	{					
		$branch_options[$branch_info->branch_id]=$branch_info->branch_name;
	}

	$savings_products_options=array(''=>"--Select--");
	foreach($savings_products_info as $savings_products)
	{
		$savings_products_options[$savings_products->id]=$savings_products->name;		
	}
	//echo form_open('register_reports/savings_refund_register');
        echo ajax_form_for_report('register_reports/ajax_savings_refund_register','#report_container',null,array('onsubmit'=>'if(validate(this)==false) return false;'));
?>
	...
	<table border="0" class="reportLayout" width="auto" cellspacing="0px" cellpadding="0">
		<tr>
                    <td>
                            ...
                            <?php echo form_dropdown('cbo_branch', $branch_options,"",'id="cbo_branch"'); ?></td>
                    <td>
			...
			<?php $txt_date_from = array('name'=>'txt_date_from','id'=>'txt_date_from','readonly'=> 'readonly');
				echo form_input($txt_date_from,set_value('txt_date_from'));?><?php echo form_error('txt_date_from'); ?></td>
		    <td>
			<label for="txt_date_to">Date To:<em>&nbsp;</em></label>			
			<?php $txt_date_to = array('name'=>'txt_date_to','id'=>'txt_date_to','readonly'=> 'readonly');
				echo form_input($txt_date_to,set_value('txt_date_to'));?><?php echo form_error('txt_date_to');?></td>
		   <td>
			<label for="cbo_product">Product:<em>&nbsp;</em></label>			
			<?php echo form_dropdown('cbo_product', $savings_products_options,"",'id="cbo_product"'); ?></td>
                    <td>
