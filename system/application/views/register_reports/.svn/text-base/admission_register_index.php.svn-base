...
function validate(form) 
{
	...
	dbd = Math.round((secondDate.valueOf()-firstDate.valueOf())/ msPerDay);
	  ...
	  if(cbo_branch == "") {
		inlineMsg('Branch','<strong>Error</strong><br />You must select one branch.',2);
		return false;
	  }
	  ...
	  { 
		...
		{
			inlineMsg('txt_date_from','<strong>Error</strong><br />You have entered an invalid Date for Date From.',2);
			return false;
		}
	if(txt_date_to == "") {
...
<?php
	...
	echo ajax_form_for_report('register_reports/ajax_admission_register','#report_container',null,array('onsubmit'=>'if(validate(this)==false) return false;'));
?>
	...
	<table border="0" class="reportLayout" width="auto" cellspacing="0px" cellpadding="0">
		<tr>
			...
			<label for="txt_date_from">Date from:<em>&nbsp;</em></label>			
				<?php $txt_date_from = array('name'=>'txt_date_from','id'=>'txt_date_from');
			...
			<td>
				...
				<?php $txt_date_to = array('name'=>'txt_date_to','id'=>'txt_date_to');
