...
function validate(form) 
{
	...
	//alert(cbo_year);
	var cbo_employee = form.cbo_employee.value;		
	//alert(cbo_year);
	...
	else if(cbo_employee == "") {		
		inlineMsg('cbo_employee','<strong>Error</strong><br />You must select an employee.',2);
		return false;
	}
	else if(cbo_product == "") {		
...
<?php
	...
	$field_worker_options=array(''=>'--Select--');
	foreach($employee_list as $employee)
	{					
		$field_worker_options[$employee->field_officer_id]=$employee->field_officer_name;
	}
...
?>
<?php echo ajax_form_for_report('regular_and_general_reports/ajax_field_worker_report','#report_container',null,array('onsubmit'=>'if(validate(this)==false) return false;'));?>
	...
	<table border="0" class="reportLayout" width="auto" cellspacing="0px" cellpadding="0">	
		<tr>	
			<td>
				...
				<?php echo form_dropdown('cbo_branch', $branches_options,"",'id="cbo_branch"');?></td>
			<td>
				<label for="cbo_employee">Field Officer:<em>&nbsp;</em></label>			
				<?php echo form_dropdown('cbo_employee', $field_worker_options,"",'id="cbo_employee"'); ?></td>
			<td>
				<label for="cbo_product">Product:<em>&nbsp;</em></label>			
				<?php echo form_dropdown('cbo_product', $product_options,"",'id="cbo_product"'); ?></td> 
			<td>
				<label for="cbo_year">Year:<em>&nbsp;</em></label>			
				<?php echo form_dropdown('cbo_year', $year_options,"",'id="cbo_year"'); ?><?php echo form_error('cbo_year'); ?></td>
			<td>
				<label for="cbo_month">Month:<em>&nbsp;</em></label>			
				<?php echo form_dropdown('cbo_month', $month_options,"",'id="cbo_month"'); ?><?php echo form_error('cbo_month'); ?></td> 
			<td>
				<label for="cbo_week_list">Week:<em>&nbsp;</em></label>			
				<?php echo form_dropdown('cbo_week_list',$week_list,"",'id="cbo_week_list"'); ?><?php echo form_error('cbo_week_list'); ?></td> 				
			<td>
				<?php echo form_submit(array('name'=>'submit','id'=>'submit','class'=>'save_button'),'Show Report');?></td></tr></table>
<?php echo form_close(); ?>
</div>
</div>
<?php echo form_close();?>