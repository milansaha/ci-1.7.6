...
<div id="filter">
	...
	<div style="border:solid 0px red;float:right;width:auto;" class="buttons">
		<?php echo anchor('loans/add',img(array('src'=>base_url().'/media/images/add.png','border'=>'0','alt'=>'Add Loans')).'Add',array('class'=>'addbutton','title'=>'Add Loans'));  ?>
	</div>	
</div>
...
<tr <?php if($i % 2 == 0){ echo 'class="evenrow";'; echo ' bgcolor="#fff"';}else{echo 'class="oddrow"';}?> >
	...
	<td align="right">
	<?php 
	echo anchor('loan_schedules/index/'.$row->id,$row->number_of_installment,'title="View loan with loan schedule"');
	?>
	</td>
	<td><?php if($row->is_authorized==0): echo $unauthorized; else: echo $authorized; endif;?></td>
	<td>
		<?php echo anchor('loan_schedules/index/'.$row->id, img(array('src'=>base_url().'media/images/view.gif','border'=>'0','alt'=>'View Loan Schedule')),array('class'=>'imglink','title'=>'View loan with loan schedule'));?>
        <?php if($row->is_authorized==0): echo anchor('loans/edit/'.$row->id,img(array('src'=>base_url().'media/images/edit.gif','border'=>'0','alt'=>'edit')),array('class'=>'imglink','title'=>'Edit'));   ?>
        <?php echo anchor('loans/delete/'.$row->id, img(array('src'=>base_url().'media/images/delete.gif','border'=>'0','alt'=>'edit')),
