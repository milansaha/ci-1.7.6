<?php
	...
	if($action=='edit')
	{
		$hidden_input=array('txt_area_id'=>$row->id);
		$class_name = 'class="formTitleBar_edit"';
	}else{$class_name = 'class="formTitleBar_add"';}
	echo form_open("po_areas/$action",'',$hidden_input);    
?>
<fieldset>
		...
		<tr>
			...
			<td class="formTitleBar">
				<div style="float:right;">
					...
					<?php echo form_button(array('name'=>'button','id'=>'button','value'=>'true','type'=>'reset','content'=>'Cancel','class'=>'cancel_buttons','onclick'=>"window.location.href='".site_url('po_areas')."'"));?>
				</div>
			</td>
		</tr>
		<tr>
			<td width="60%">
				<div class="formContainer">
					<ol>
						...
						<li>
							<label for="cbo_branch_list">Branch:<span class="required_field_indicator">*</span></label>
                            <div class="form_input_container">
							<?php echo form_multiselect('cbo_branch_list[]', $branch_infos, set_value('cbo_branch_list', (isset ($selected_branch)?$selected_branch:'')),'id="cbo_branch_list" class="input_select" size="10"'); ?>
							...
							<?php echo form_error('cbo_branch_list[]'); ?>
                            </div>
						</li>
					</ol>
				</div>
